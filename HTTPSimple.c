...
int verify_callback(int preverify, X509_STORE_CTX *x509_ctx) {
    ...
    // Получение CN из сертификата
    X509_NAME *subject_name = X509_get_subject_name(cert);
    X509_NAME_get_text_by_NID(subject_name, NID_commonName, buf, 256);
    printf("Common Name = %s\n", buf);

    // Подключение к базе данных для проверки пользователя
    sqlite3 *db;
    sqlite3_stmt *stmt;
    ...
    // Запрос пользователя в базе данных
    const char *sql = "SELECT * FROM users WHERE common_name = ?";
